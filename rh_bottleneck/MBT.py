...
from rh_bottleneck.layers.encoders import DiffTransformerEncoder, FusionTransformerEncoder
import torch
...
class Alternately_Attention_Bottlenecks(nn.Module):
    ...
    def get_embeddings(self, data):
        """Extract embeddings for visualization purposes"""
        self.eval()  # Set to evaluation mode
        with torch.no_grad():
            # Run forward pass but return only the embeddings (multi_scale_bottlenecks[-1])
            logit, sc_x, fc_x, embeddings = self.forward(data)
            return embeddings.squeeze(1)  # Return graph-level embeddings
